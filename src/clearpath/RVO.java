...
public class RVO
{
     ...
     */
    static private boolean avoidCollisionWithLine(List<Line> lines,
                                                  int lineID, 
                                                  double maxSpeed,
                                                  ...
                                                  Double[] newVelocity) 
    {
        ...
        // Check if there is any collision between obstacle with lineID and other
        // obstacles before it
        for (int i = 0; i < lineID; ++i)
        {
            final double denominator = det2D(lines.get(lineID).direction, lines.get(i).direction);
            final double numerator   = det2D(lines.get(i).direction,
                                             vectorSubstract(lines.get(lineID).point, lines.get(i).point));
